...
class DemoTestCase(TestCase):
    ...
    def test_one(self):
        ...
        sw_eng = ProfessorCourseMapping.objects.get(course__course_title="Software Engineering")
        sw_eng_scms = StudentCourseMapping.objects.filter(course=sw_eng.course)
        ranked = rank_students(sw_eng_scms, sw_eng)

        size = 3
        groups = group_students(size, ranked)
        self.assertEqual(len(groups), len(ranked)/size)
        for group in groups:
